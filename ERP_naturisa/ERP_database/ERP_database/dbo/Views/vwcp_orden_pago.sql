...
as
SELECT        OP.IdEmpresa, OP.IdOrdenPago, OP.IdTipo_op, OP.IdTipo_Persona, OP.IdPersona, OP.IdEntidad, OP.Fecha, OP.IdEstadoAprobacion, OP.IdFormaPago, 
                         OP.Fecha_Pago, OP.IdBanco, OP.Estado, dbo.tb_persona.pe_nombreCompleto, CAN.Total_OP, ISNULL(PAGO.Total_cancelado, 0) AS Total_cancelado, 
                         round(CAN.Total_OP - ISNULL(PAGO.Total_cancelado, 0),2) AS Saldo, OP.Observacion, OP.IdTipoFlujo, dbo.ba_TipoFlujo.Descricion AS nom_tipoFlujo, 
                         CASE WHEN round(CAN.Total_OP - ISNULL(PAGO.Total_cancelado, 0),2) > 0 THEN 'PENDIENTE' WHEN round(CAN.Total_OP - ISNULL(PAGO.Total_cancelado, 0) ,2)
...
Begin DesignProperties = 
   Begin PaneConfigurations = 
      Begin PaneConfiguration = 0
         NumPanes = 4
         Configuration = "(H (1[19] 4[12] 2[54] 3) )"
      End
   ...
   Begin DataPane = 
      ...
      Begin ColumnWidths = 20
         ...
         Width = 1500
         Width = 1500
         Width = 1', @level0type = N'SCHEMA', @level0name = N'dbo', @level1type = N'VIEW', @level1name = N'vwcp_orden_pago';
