...
SELECT        fa_factura.IdEmpresa, fa_factura.IdSucursal, fa_factura.IdBodega, fa_factura.vt_tipoDoc AS IdTipoDocumento, CASE WHEN dbo.fa_factura.vt_NumFactura IS NULL 
                         THEN dbo.fa_factura.vt_tipoDoc + '#' + CAST(dbo.fa_factura.IdCbteVta AS varchar(20)) ELSE dbo.fa_factura.vt_tipoDoc + '-' + isnull(dbo.fa_factura.vt_serie1, '') 
                         + '-' + isnull(dbo.fa_factura.vt_serie2, '') + '-' + isnull(dbo.fa_factura.vt_NumFactura, '') + '/' + CAST(dbo.fa_factura.IdCbteVta AS varchar(20)) END AS numDocumento, 
                         fa_factura.IdCliente, tb_persona.pe_nombreCompleto AS nombreCliente, fa_factura.vt_fecha, tb_Calendario.IdCalendario, tb_Calendario.AnioFiscal, 
                         tb_Calendario.NombreMes, tb_Calendario.NombreCortoFecha, in_Producto.IdProducto, in_Producto.pr_descripcion AS nombreProducto, fa_factura_det.vt_cantidad, 
                         fa_factura_det.vt_PrecioFinal, fa_factura_det.vt_Subtotal, fa_factura_det.vt_iva, fa_factura_det.vt_total, tb_sucursal.Su_Descripcion, 
                         vwin_Cate_Lin_Grup_SubGrup.IdCategoria, vwin_Cate_Lin_Grup_SubGrup.IdLinea, vwin_Cate_Lin_Grup_SubGrup.IdGrupo, 
                         vwin_Cate_Lin_Grup_SubGrup.IdSubgrupo, vwin_Cate_Lin_Grup_SubGrup.ca_Categoria, vwin_Cate_Lin_Grup_SubGrup.nom_linea, 
                         vwin_Cate_Lin_Grup_SubGrup.nom_grupo, vwin_Cate_Lin_Grup_SubGrup.nom_subgrupo, fa_cliente_tipo.Idtipo_cliente, fa_cliente_tipo.Descripcion_tip_cliente, 
                         fa_factura.vt_Observacion, fa_Vendedor.IdVendedor, fa_Vendedor.Ve_Vendedor AS Vendedor
FROM            fa_factura INNER JOIN
                         tb_bodega ON fa_factura.IdEmpresa = tb_bodega.IdEmpresa AND fa_factura.IdSucursal = tb_bodega.IdSucursal AND 
                         fa_factura.IdBodega = tb_bodega.IdBodega INNER JOIN
                         tb_sucursal ON tb_bodega.IdEmpresa = tb_sucursal.IdEmpresa AND tb_bodega.IdSucursal = tb_sucursal.IdSucursal INNER JOIN
                         fa_cliente ON fa_factura.IdEmpresa = fa_cliente.IdEmpresa AND fa_factura.IdCliente = fa_cliente.IdCliente AND tb_sucursal.IdEmpresa = fa_cliente.IdEmpresa AND 
                         ...
                         fa_Vendedor ON fa_factura.IdEmpresa = fa_Vendedor.IdEmpresa AND fa_factura.IdVendedor = fa_Vendedor.IdVendedor AND 
                         fa_factura.IdEmpresa = fa_Vendedor.IdEmpresa AND fa_factura.IdVendedor = fa_Vendedor.IdVendedor LEFT OUTER JOIN
...
SELECT        fa_notaCreDeb.IdEmpresa, fa_notaCreDeb.IdSucursal, fa_notaCreDeb.IdBodega, (CASE dbo.fa_TipoNota.Tipo WHEN 'C' THEN 'NTCR' ELSE 'NTDB' END) 
                         ...
                         END AS numDocumento, fa_notaCreDeb.IdCliente, tb_persona.pe_nombreCompleto AS nombreCliente, fa_notaCreDeb.no_fecha, tb_Calendario.IdCalendario, 
                         tb_Calendario.AnioFiscal, tb_Calendario.NombreMes, tb_Calendario.NombreCortoFecha, in_Producto.IdProducto, in_Producto.pr_descripcion AS nombreProducto, 
                         (CASE dbo.fa_TipoNota.Tipo WHEN 'C' THEN (dbo.fa_notaCreDeb_det.sc_cantidad * - 1) ELSE dbo.fa_notaCreDeb_det.sc_cantidad END) AS vt_cantidad, 
                         ...
                         (CASE dbo.fa_TipoNota.Tipo WHEN 'C' THEN (dbo.fa_notaCreDeb_det.sc_iva * - 1) ELSE dbo.fa_notaCreDeb_det.sc_iva END) AS vt_iva, 
                         (CASE dbo.fa_TipoNota.Tipo WHEN 'C' THEN (dbo.fa_notaCreDeb_det.sc_total * - 1) ELSE dbo.fa_notaCreDeb_det.sc_total END) AS vt_total, 
                         tb_sucursal.Su_Descripcion, vwin_Cate_Lin_Grup_SubGrup.IdCategoria, vwin_Cate_Lin_Grup_SubGrup.IdLinea, vwin_Cate_Lin_Grup_SubGrup.IdGrupo, 
                         vwin_Cate_Lin_Grup_SubGrup.IdSubgrupo, vwin_Cate_Lin_Grup_SubGrup.ca_Categoria, vwin_Cate_Lin_Grup_SubGrup.nom_linea, 
                         vwin_Cate_Lin_Grup_SubGrup.nom_grupo, vwin_Cate_Lin_Grup_SubGrup.nom_subgrupo, fa_cliente_tipo.Idtipo_cliente, fa_cliente_tipo.Descripcion_tip_cliente, 
                         fa_notaCreDeb.sc_observacion, fa_Vendedor.IdVendedor, fa_Vendedor.Ve_Vendedor AS Vendedor
FROM            fa_notaCreDeb INNER JOIN
                         ...
                         fa_notaCreDeb.IdEmpresa = fa_Vendedor.IdEmpresa AND fa_notaCreDeb.IdVendedor = fa_Vendedor.IdVendedor LEFT OUTER JOIN
                         vwin_Cate_Lin_Grup_SubGrup ON vwin_Cate_Lin_Grup_SubGrup.IdEmpresa = in_Producto.IdEmpresa AND 
                         vwin_Cate_Lin_Grup_SubGrup.IdCategoria = in_Producto.IdCategoria AND vwin_Cate_Lin_Grup_SubGrup.IdSubgrupo = in_Producto.IdSubGrupo
GO
