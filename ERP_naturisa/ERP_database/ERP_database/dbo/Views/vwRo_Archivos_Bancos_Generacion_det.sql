create view vwRo_Archivos_Bancos_Generacion_det as 
SELECT        dbo.ro_empleado.IdEmpleado, dbo.tb_persona.pe_cedulaRuc AS CedulaRuc, dbo.tb_persona.pe_nombreCompleto AS NombreCompleto, dbo.ro_rol_detalle.IdRubro, 
                         ...
                         dbo.ro_Departamento.IdDepartamento, dbo.tb_banco.ba_descripcion, ISNULL
                             ((SELECT        SUM(ISNULL(Valor, 0)) AS Expr1
                                 FROM            dbo.vwro_archivos_bancos_generacion_x_empleado AS P
                                 WHERE        (IdEmpresa = dbo.ro_rol_detalle.IdEmpresa) AND (IdNomina = dbo.ro_rol_detalle.IdNominaTipo) AND 
                                                          (IdNominaTipo = dbo.ro_rol_detalle.IdNominaTipoLiqui) AND (IdPeriodo = dbo.ro_rol_detalle.IdPeriodo) AND 
                                                          (IdEmpleado = dbo.ro_rol_detalle.IdEmpleado)
                             ...
                             (SELECT        SUM(ISNULL(Valor, 0)) AS Expr1
                               ...
                               WHERE        (IdEmpresa = dbo.ro_rol_detalle.IdEmpresa) AND (IdNomina = dbo.ro_rol_detalle.IdNominaTipo) AND 
                                                         (IdNominaTipo = dbo.ro_rol_detalle.IdNominaTipoLiqui) AND (IdPeriodo = dbo.ro_rol_detalle.IdPeriodo) AND 
                                                         (IdEmpleado = dbo.ro_rol_detalle.IdEmpleado)
                               GROUP BY IdEmpresa, IdNomina, IdNominaTipo, IdPeriodo, IdEmpleado), dbo.ro_rol_detalle.Valor) AS PendientePago, 
                         dbo.ro_archivos_bancos_generacion.IdArchivo
FROM            dbo.ro_rubro_tipo INNER JOIN
                         ...
                         dbo.ro_rol_detalle.IdNominaTipo = dbo.ro_archivos_bancos_generacion.IdNomina AND 
                         dbo.ro_rol_detalle.IdNominaTipoLiqui = dbo.ro_archivos_bancos_generacion.IdNominaTipo AND 
                         dbo.ro_rol_detalle.IdPeriodo = dbo.ro_archivos_bancos_generacion.IdPeriodo