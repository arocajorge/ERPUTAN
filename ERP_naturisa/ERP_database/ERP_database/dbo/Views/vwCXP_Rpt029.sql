...
SELECT        IdEmpresa, em_nombre, pr_codigo, pr_nombre, Factura, NumRetencion, AVG(co_Por_iva) AS co_Por_iva, AVG(co_valoriva) AS co_valoriva, AVG(co_subtotal_iva) AS co_subtotal_iva, AVG(co_subtotal_siniva) 
                         AS co_subtotal_siniva, fecha, SUM(Base_Fuente) AS Base_Fuente, AVG(co_subtotal_iva) + AVG(co_subtotal_siniva) - SUM(Base_Fuente) AS Diferencia, Tiene_retencion, re_tipoRet
FROM            (SELECT      dbo.tb_empresa.IdEmpresa, dbo.tb_empresa.em_nombre, dbo.cp_proveedor.pr_codigo, dbo.cp_proveedor.pr_nombre, dbo.cp_orden_giro.co_serie + '-' + dbo.cp_orden_giro.co_factura AS Factura, 
            ...
            dbo.cp_retencion_det.re_tipoRet = 'RTF' THEN 'FACTURAS CON RETENCION/VALIDAS' ELSE 'FACTURAS SIN RETENCION' END AS Tiene_retencion, dbo.cp_retencion_det.re_tipoRet
...
WHERE       (dbo.cp_orden_giro.co_vaCoa = 'S') AND (dbo.cp_retencion_det.re_tipoRet = 'RTF') --OR (dbo.cp_retencion_det.re_tipoRet = 'IVA' AND dbo.cp_retencion.re_Tiene_RFuente = 'N'))
) AS Data
