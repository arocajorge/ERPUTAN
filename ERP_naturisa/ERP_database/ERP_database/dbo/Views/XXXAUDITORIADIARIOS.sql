...
SELECT dbo.ct_cbtecble.IdEmpresa, dbo.ct_cbtecble_tipo.tc_TipoCbte AS TIPOCOMPROBANTE, dbo.ct_cbtecble.IdCbteCble AS NUMERO, CASE WHEN ct_cbtecble.cb_Estado = 'A' THEN 'ACTIVO' ELSE 'INACTIVO' END AS ESTADO, 
                  dbo.ct_cbtecble_det.IdCtaCble AS CODIGOCUENTA, REPLACE(REPLACE(dbo.ct_plancta.pc_Cuenta, CHAR(10), ''), CHAR(13), '') AS NOMCUENTA, REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(dbo.ct_cbtecble.cb_Observacion)), 
                  CHAR(10), ''), CHAR(13), ''), '  ', '') AS GLOSA, CASE WHEN ct_cbtecble_det.dc_Valor > 0 THEN 'DB' ELSE 'CR' END AS SENTIDO, ABS(dbo.ct_cbtecble_det.dc_Valor) AS MONTO, dbo.ct_cbtecble.cb_Fecha AS FECHACONTABILIZACION, 
...
WHERE  (dbo.ct_cbtecble.IdEmpresa = 1) AND (dbo.ct_cbtecble.cb_Fecha BETWEEN datefromparts(2019, 1, 1) AND datefromparts(2019, 8, 31))