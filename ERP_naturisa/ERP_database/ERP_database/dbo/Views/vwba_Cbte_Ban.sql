...
SELECT B.tc_TipoCbte, C.ba_descripcion, '' AS NombreProveedor, Ba.IdEmpresa, Ba.IdCbteCble, Ba.IdTipocbte, Ba.Cod_Cbtecble, Ba.IdPeriodo, Ba.IdBanco, Ba.IdProveedor, Ba.cb_Fecha, Ba.cb_Observacion, Ba.cb_secuencia, Ba.cb_Valor, 
                  ...
                  tb_persona_1.pe_nombreCompleto IS NOT NULL THEN tb_persona_1.pe_nombreCompleto ELSE dbo.tb_persona.pe_nombreCompleto END) AS Beneficiario, 
				  
				  tb_persona_2.IdTipoDocumento, 
                  ...
                  tb_persona_1.pe_nombreCompleto IS NOT NULL THEN tb_persona_1.pe_cedulaRuc ELSE dbo.tb_persona.pe_cedulaRuc END) AS pe_cedulaRuc, 
                  dbo.ba_Cbte_Ban_tipo_x_ct_CbteCble_tipo.CodTipoCbteBan, ISNULL(dbo.vwba_Banco_Estado_Cheques.Estado_Conciliacion, 'NO CONCILIADO') AS Estado_Conciliacion, 
                  CASE WHEN Ba.IdEstado_Preaviso_ch_cat = '' THEN 'ES_CH_XPREAVISO_CH' ELSE isnull(Ba.IdEstado_Preaviso_ch_cat, 'ES_CH_XPREAVISO_CH') END AS IdEstado_Preaviso_ch_cat, Ba.IdEstado_cheque_cat
...
GROUP BY B.tc_TipoCbte, C.ba_descripcion, Ba.IdEmpresa, Ba.IdCbteCble, Ba.IdTipocbte, Ba.Cod_Cbtecble, Ba.IdPeriodo, Ba.IdBanco, Ba.IdProveedor, Ba.cb_Fecha, Ba.cb_Observacion, Ba.cb_secuencia, Ba.cb_Valor, Ba.cb_Cheque, 
                  ...
                  ISNULL(Ba.IdEstado_Cbte_Ban_cat, ''), dbo.vwba_Estado_cbte_ban.ca_descripcion, tb_persona_2.IdTipoDocumento, tb_persona_2.pe_cedulaRuc, dbo.ba_Cbte_Ban_tipo_x_ct_CbteCble_tipo.CodTipoCbteBan, 
                  ISNULL(dbo.vwba_Banco_Estado_Cheques.Estado_Conciliacion, 'NO CONCILIADO'), Ba.IdEstado_cheque_cat, Ba.IdEstado_Preaviso_ch_cat