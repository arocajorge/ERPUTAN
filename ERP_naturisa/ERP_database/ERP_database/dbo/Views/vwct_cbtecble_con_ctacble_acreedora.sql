...
FROM     (SELECT cbte_d.IdEmpresa, cbte_d.IdTipoCbte, cbte_d.IdCbteCble, cbte_d.IdCtaCble AS IdCtaCble_Acreedora
                  ...
                  WHERE   OG.Estado = 'A' AND (cbte_d.dc_Valor < 0) AND EXISTS
                                        (SELECT IdEmpresa
                  ...
                  WHERE  cp_nota_debcre.ESTADO = 'A' AND (cbte_d.dc_Valor < 0) AND (dbo.cp_nota_DebCre.DebCre = 'D') AND EXISTS
                                        (SELECT IdEmpresa
                                         FROM      dbo.cp_proveedor_clase AS cl_pr
                                         WHERE   (IdEmpresa = cbte_d.IdEmpresa) AND (IdCtaCble_CXP = cbte_d.IdCtaCble))
                  UNION ALL
                  ...
                  FROM     dbo.cp_orden_pago_det INNER JOIN
                                    ...
                                    dbo.ct_cbtecble_det AS A ON dbo.cp_orden_pago_det.IdEmpresa_cxp = A.IdEmpresa AND dbo.cp_orden_pago_det.IdTipoCbte_cxp = A.IdTipoCbte AND dbo.cp_orden_pago_det.IdCbteCble_cxp = A.IdCbteCble INNER JOIN
                                    ...
                                    dbo.cp_orden_pago.IdPersona = dbo.vwtb_persona_beneficiario.IdPersona AND dbo.cp_orden_pago.IdEntidad = dbo.vwtb_persona_beneficiario.IdEntidad AND A.IdCtaCble = dbo.vwtb_persona_beneficiario.IdCtaCble
                  WHERE  CP_ORDEN_PAGO.ESTADO = 'A' AND (A.dc_Valor < 0) AND (dbo.cp_orden_pago.IdTipo_op <> 'FACT_PROVEE')) AS Querry
