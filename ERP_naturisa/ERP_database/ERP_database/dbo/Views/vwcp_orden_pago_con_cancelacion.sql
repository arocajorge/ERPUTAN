...
FROM            (SELECT        A.IdEmpresa, A.IdTipo_op, SUBSTRING(D .Descripcion, 1, 3) + '#' + C.co_serie + ' -' + C.co_factura AS Referencia, SUBSTRING(D .Descripcion, 1, 3) 
                                                    + '#' + CAST(CAST(C.co_factura AS NUMERIC) AS VARCHAR) AS Referencia2, A.IdOrdenPago, B.Secuencia AS Secuencia_OP, 
                                                    'PROVEE' AS IdTipoPersona, Pr.IdPersona, C.IdProveedor AS IdEntidad, A.Fecha AS Fecha_OP, C.co_fechaOg AS Fecha_Fa_Prov, 
                                                    C.co_FechaFactura_vct AS Fecha_Venc_Fac_Prov, A.Observacion, Pr.pr_nombre AS Nom_Beneficiario, Pr.pr_girar_cheque_a AS Girar_Cheque_a, 
                                                    C.co_valorpagar AS Valor_a_pagar, B.Valor_a_pagar AS Valor_estimado_a_pagar_OP, ISNULL(Can.Total_cancelado, 0) AS Total_cancelado_OP, 
                                                    round(B.Valor_a_pagar - ISNULL(Can.Total_cancelado, 0),2) AS Saldo_x_Pagar_OP, B.IdEstadoAprobacion, B.IdFormaPago, B.Fecha_Pago, 
                          ...
                          SELECT        A.IdEmpresa, A.IdTipo_op, 'ND' + '#' + cast(cp_nota_DebCre.IdCbteCble_Nota AS varchar(20)) + cp_nota_DebCre.cn_observacion AS Referencia, 
                                                   ...
                                                   ELSE 'ND' + '#' + CAST(CAST(cp_nota_DebCre.cod_nota AS NUMERIC) AS varchar(20)) END AS Referencia2, A.IdOrdenPago, 
                                                   B.Secuencia AS Secuencia_OP, 'PROVEE' AS IdTipoPersona, Pr.IdPersona, cp_nota_DebCre.IdProveedor AS IdEntidad, A.Fecha AS Fecha_OP, 
                                                   cp_nota_DebCre.cn_fecha, cp_nota_DebCre.cn_Fecha_vcto, A.Observacion, Pr.pr_nombre AS Nom_Beneficiario, 
                                                   Pr.pr_girar_cheque_a AS Girar_Cheque_a, cp_nota_DebCre.cn_total Valor_a_pagar, B.Valor_a_pagar AS Valor_estimado_a_pagar_OP, 
                                                   ISNULL(Can.Total_cancelado, 0) AS Total_cancelado_OP, round(B.Valor_a_pagar - ISNULL(Can.Total_cancelado, 0),2) AS Saldo_x_Pagar_OP, 
                                                   B.IdEstadoAprobacion, B.IdFormaPago, B.Fecha_Pago, vwct_cbtecble_con_ctacble_acreedora.IdCtaCble_Acreedora AS IdCtaCble, 
                                                   cp_nota_DebCre.IdCentroCosto, NULL IdSubCentroCosto, cp_nota_DebCre.IdCbteCble_Nota Cbte_cxp, cp_nota_DebCre.Estado, 
                          ...
                          SELECT        OP.IdEmpresa, OP.IdTipo_op, 'OP#' + CAST(OP.IdOrdenPago AS varchar(20)) AS Referencia, 'OP#' + CAST(OP.IdOrdenPago AS varchar(20)) 
                                                   ...
                                                   round(OPD.Valor_a_pagar - ISNULL(CAN_1.Total_cancelado, 0),2) AS Saldo_x_pagar_OP, OPD.IdEstadoAprobacion, OPD.IdFormaPago, OPD.Fecha_Pago, 
                          ...
                          WHERE        NOT EXISTS
                                                       (SELECT        og.IdEmpresa
                                                         ...
                                                         WHERE        og.IdEmpresa = opd.IdEmpresa_cxp AND og.IdTipoCbte_Ogiro = opd.IdTipoCbte_cxp AND og.IdCbteCble_Ogiro = opd.IdCbteCble_cxp) AND 
                                                   NOT EXISTS
                                                       (SELECT        nd.IdEmpresa
                          ...
                          SELECT        C.IdEmpresa, 'FACT_PROVEE' AS IdTipo_op, SUBSTRING(D .Descripcion, 1, 3) + '#' + C.co_serie + ' -' + C.co_factura AS Referencia, 
                                                   'FA#:' + CAST(CAST(C.co_factura AS NUMERIC) AS VARCHAR(20)) AS Referencia2, NULL AS IdOrdenPago, NULL AS Secuencia_OP, 
                                                   'PROVEE' AS IdTipoPersona, Pr.IdPersona, C.IdProveedor AS IdEntidad, C.co_fechaOg AS Fecha_OP, C.co_fechaOg AS Fecha_Fa_Prov, 
                                                   ...
                                                   round(C.co_valorpagar - ISNULL(Total_OP.Valor_a_pagar, 0),2) AS Saldo_x_Pagar_OP, 'APRO' AS IdEstadoAprobacion, 'EFEC' AS IdFormaPago, NULL 
