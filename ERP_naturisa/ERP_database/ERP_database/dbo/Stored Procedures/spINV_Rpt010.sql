-- exec [dbo].[spINV_Rpt010] 1,2,2,29,29,2372,2372,'11/01/2018','20/01/2018','admin',0,1
...
SELECT * FROM (
				SELECT        det.IdEmpresa, det.IdSucursal, det.IdBodega, det.IdMovi_inven_tipo, in_Ing_Egr_Inven_det.IdNumMovi, det.Secuencia, det.IdProducto
						...
						iif((in_INV_Rpt010.Saldo_ini_cant + sum(isnull(det.dm_cantidad,0))
						over(partition by 
						det.IdEmpresa, det.IdSucursal,det.IdBodega, det.IdProducto
						order by det.IdEmpresa asc, det.IdSucursal asc ,det.IdBodega asc, det.IdProducto asc,cab.cm_fecha asc,det.dm_cantidad desc, det.IdNumMovi ASC,det.Secuencia ASC
						...
						rows unbounded preceding) as Saldo_cost
						 ...
						 ,in_INV_Rpt010.Saldo_fin_cant, case when round(in_INV_Rpt010.Saldo_fin_cost,2) < 0.1 and round(in_INV_Rpt010.Saldo_fin_cant,2) < 0.1 then 0 ELSE (in_INV_Rpt010.Saldo_fin_cost / iif(in_INV_Rpt010.Saldo_fin_cant=0,1,in_INV_Rpt010.Saldo_fin_cant)) END Cost_prom_fin, round(in_INV_Rpt010.Saldo_fin_cost,2)Saldo_fin_cost
