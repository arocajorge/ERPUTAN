...
BEGIN
 ...
 SELECT        dbo.ro_cargo.ca_descripcion AS cargo, dbo.ro_Departamento.de_descripcion AS departamento, dbo.ro_empleado.IdEmpresa, dbo.ro_empleado.IdEmpleado, 
                         ...
                         dbo.tb_provincia.IdProvincia, dbo.tb_ciudad.IdCiudad, 
                                                 (select sum( Valor) from ro_Ing_Egre_x_Empleado where IdEmpleado=ro_empleado.IdEmpleado and IdRubro='24' and SUBSTRING( cast( IdPeriodo as varchar(10)),1,4)=@Anio) as tot_Sueldo_Ganado,-- obtengo el total de sueldo ganados
                                                 (select sum( Valor) from ro_Ing_Egre_x_Empleado where IdEmpleado=ro_empleado.IdEmpleado and IdRubro='289' and SUBSTRING( cast( IdPeriodo as varchar(10)),1,4)=@Anio) as tot_decimo_cuarto,-- total de decimo cuarto
                         (select sum( Valor) from ro_Ing_Egre_x_Empleado where IdEmpleado=ro_empleado.IdEmpleado and IdRubro='290' and SUBSTRING( cast( IdPeriodo as varchar(10)),1,4)=@Anio) as tot_decimo_tercero,-- total de decimo tercero 
                                                 (select sum( Valor) from ro_Ing_Egre_x_Empleado where IdEmpleado=ro_empleado.IdEmpleado and IdRubro='296' and SUBSTRING( cast( IdPeriodo as varchar(10)),1,4)=@Anio) as tot_fondo_reserva,-- total de fondos de reservas
                                                 (select sum( Valor) from ro_Ing_Egre_x_Empleado where IdEmpleado=ro_empleado.IdEmpleado and IdRubro in(7,8,9) and SUBSTRING(cast( IdPeriodo as varchar(10)),1,4)=@Anio) as tot_horasExtras,-- total de horas extras
...
FROM            dbo.tb_persona INNER JOIN
                         ...
                         dbo.ro_cargo ON dbo.ro_empleado.IdEmpresa = dbo.ro_cargo.IdEmpresa AND dbo.ro_empleado.IdCargo = dbo.ro_cargo.IdCargo and 
                                                 dbo.ro_empleado.IdEmpresa=@Idempresa
END