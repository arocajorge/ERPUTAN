CREATE VIEW [Fj_servindustrias].[vwCOMP_FJ_Rpt001]
	...
	SELECT        OC.IdEmpresa, OC.IdSucursal, OC.IdOrdenCompra, OC.IdProveedor, OC.oc_NumDocumento, OC.Tipo, OC.IdTerminoPago, OC.oc_plazo AS Plazo, 
                         ...
                         depar.de_descripcion AS departamento, OC_det.Secuencia, OC_det.IdProducto, OC_det.do_Cantidad AS cantidad, OC_det.do_precioCompra AS precio, 
                         OC_det.do_porc_des AS por_desc, OC_det.do_descuento AS valor_descuento, OC_det.do_subtotal AS subtotal, OC_det.do_iva AS iva, OC_det.do_total AS total, 
                         OC_det.do_peso AS peso, Prod.pr_codigo AS cod_producto, Prod.pr_descripcion AS nom_producto, sucu.Su_Descripcion AS sucursal, empr.em_nombre AS empresa, 
                         ...
                         dbo.com_solicitante.nom_solicitante, dbo.com_Motivo_Orden_Compra.Descripcion, empr.em_telefonos, 'fjservindustrias@gmail.com' AS mail_empresa, 
                         sucu.Su_Direccion AS Direccion_sucu, sucu.Su_Telefonos AS Telef_Sucursal, prove.contacto AS Contacto_Prov, per_prov.pe_correo AS mail_prove, 
                         OC.IdEstadoAprobacion_cat, dbo.com_TerminoPago.Descripcion AS Nom_TerminoPago
FROM            dbo.com_solicitante RIGHT OUTER JOIN
                         ...
                         OC_det.IdCentroCosto_sub_centro_costo = dbo.ct_centro_costo_sub_centro_costo.IdCentroCosto_sub_centro_costo ON 
                         dbo.in_UnidadMedida.IdUnidadMedida = OC_det.IdUnidadMedida ON dbo.com_solicitante.IdEmpresa = OC.IdEmpresa AND 
                         dbo.com_solicitante.IdSolicitante = OC.IdSolicitante LEFT OUTER JOIN
