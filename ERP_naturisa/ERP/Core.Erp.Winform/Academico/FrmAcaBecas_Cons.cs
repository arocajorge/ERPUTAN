...
{
    public partial class FrmAcaBecas_Cons : Form
    {
        #region "Variable"
        ...
        Aca_Beca_Info InfoBecas = new Aca_Beca_Info();

        FrmAcaBecas_Mant frm;



        List<Aca_Beca_Info> listaBecas = new List<Aca_Beca_Info>();
        Aca_Beca_Bus BusBecas = new Aca_Beca_Bus();


        #endregion

        #region "Constructor"
        public FrmAcaBecas_Cons()
        {
            InitializeComponent();
        ...
        #region "Load"
        private void FrmAcaBecas_Cons_Load(object sender, EventArgs e)
        {
            try
            {
                llenar_grid();
        ...
        private void llenar_grid()
        {
            try
            {
                listaBecas = BusBecas.Get_List_Beca(1);
                gridControlBecas.DataSource = listaBecas;
        ...
        private void llama_frm(Cl_Enumeradores.eTipo_action Accion)
        {
            try
            {
                ...
                frm = new FrmAcaBecas_Mant();
                frm.MdiParent = this.MdiParent;
                frm.event_FrmAcaBecas_Mant_FormClosing += frm_event_FrmAcaBecas_Mant_FormClosing;
                if (Accion != Cl_Enumeradores.eTipo_action.grabar)
                {
                    frm.set_Info(InfoBecas);
                }
        ...
        void frm_event_FrmAcaBecas_Mant_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                llenar_grid();
        ...
        {
            this.gridControlBecas.ShowPrintPreview();
        }

        private void gridViewBecas_RowStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowStyleEventArgs e)
        {
            try
            {
                var data = gridViewBecas.GetRow(e.RowHandle) as Aca_Beca_Info;
                if (data == null)
                    return;
                if (data.estado == "I")
