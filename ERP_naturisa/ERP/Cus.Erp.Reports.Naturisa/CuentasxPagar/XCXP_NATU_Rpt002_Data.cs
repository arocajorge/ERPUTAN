            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(ConexionERP.GetConnectionString()))
                {
                    ...
                    command.CommandText = "DECLARE @IdEmpresa int = "+IdEmpresa.ToString()+","
                                        ...
                                        +" cast(A.Fecha_Pago AS date), A.Observacion, A.IdPersona, A.IdEntidad, ltrim(rtrim(A.pe_nombreCompleto)) pe_nombreCompleto, A.Total_OP, 0 AS Iva, A.Total_OP, A.Total_cancelado, A.Saldo, 0 AS IdClaseProveedor, '' AS descr, "
                                        +" SU.IdSucursal, SU.Su_Descripcion"
                                        ...
                                        +" WHERE        A.IdTipo_op <> 'FACT_PROVEE' AND A.Estado = 'A' AND round(a.saldo,2) != 0"
