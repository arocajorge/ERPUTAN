...
{
    public partial class XINV_NAT_Rpt007_Rpt : DevExpress.XtraReports.UI.XtraReport
    {
        cl_parametrosGenerales_Bus param = cl_parametrosGenerales_Bus.Instance;
        tb_sis_Log_Error_Vzen_Bus Log_Error_bus = new tb_sis_Log_Error_Vzen_Bus();
        List<XINV_NAT_Rpt007_Info> lst_reporte = new List<XINV_NAT_Rpt007_Info>();
        XINV_NAT_Rpt007_Bus bus_reporte = new XINV_NAT_Rpt007_Bus();
        List<int> lst_bodega = new List<int>();
        List<string> lst_subcentro = new List<string>();

        public XINV_NAT_Rpt007_Rpt()
        {
            InitializeComponent();
        }
        ...
        private void XINV_NAT_Rpt007_Rpt_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            try
            {
            ...
            catch (Exception ex)
            {
                Log_Error_bus.Log_Error(ex.ToString());
                Core.Erp.Info.Log_Exception.LoggingManager.Logger.Log(Core.Erp.Info.Log_Exception.LoggingCategory.Error, ex.Message);
                throw new Core.Erp.Info.Log_Exception.DalException(string.Format("", "XINV_NAT_Rpt007_Rpt_BeforePrint", ex.Message), ex) { EntityType = typeof(XINV_NAT_Rpt003_Rpt) };
            }
        }
